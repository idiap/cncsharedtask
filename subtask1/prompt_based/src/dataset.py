...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        self.args = args
        self.processor = TextClassificationProcessor(args.data_dir)
        ...
        assert mode in ["train", "dev", "test", "test_competition"]
        ...
        if args.prompt:
            ...
            for key in self.label_to_word:
                ...
                if self.label_to_word[key][0] not in ["<", "[", ".", ","]:
                    ...
                    self.label_to_word[key] = tokenizer.convert_tokens_to_ids(
        ...
        with FileLock(lock_path):
            ...
            if os.path.exists(cached_features_file) and not args.overwrite_cache:
                ...
                self.support_examples = self.processor.get_train_examples()

                if mode == "dev":
                    self.query_examples = self.processor.get_dev_examples()
                elif mode == "test":
                    self.query_examples = self.processor.get_test_examples()
                elif mode == "test_competition":
                    self.query_examples = self.processor.get_test_competition_examples()
                else:
        ...
        if self.use_demo and args.demo_filter:
            ...
            if split_name not in ["train", "dev", "test", "test_competition"]:
            ...
            for query_idx in range(len(self.query_examples)):
                ...
                if self.use_demo and args.demo_filter:
                    ...
                    for support_idx, score in sim_score:
                        if (
                            count_each_label[self.support_examples[support_idx].label]
                            < limit_each_label
                        ):
                            count_each_label[
    ...
    def __getitem__(self, i):
        if self.features is None:
            query_idx, context_indices, _ = self.example_idx[i]
            # The input (query) example
            example = self.query_examples[query_idx]
            # The demonstrations
            supports = self.select_context(
    ...
    def convert_fn(
        ...
        if not use_demo:
            ...
            support_by_label = [[] for _ in range(len(label_map))]
