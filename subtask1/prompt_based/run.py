...
from src.models import (
    ...
    DebertaForPromptFinetuning,
    Debertav2ForPromptFinetuning,
    RobertaForPromptFinetuning,
    resize_token_type_embeddings,
)
from src.processors import acc_and_f1
...
class DynamicTrainingArguments(TrainingArguments):
    ...
    save_logit: bool = field(
        default=False,
        metadata={"help": "Save test file logit with name $MODEL_ID-$ARRAY_ID.npy"},
    )
...
def main():
    ...
    # TODO: if test-competition is True
    test_competition_dataset = (
        FewShotDataset(
            data_args,
            tokenizer=tokenizer,
            mode="test_competition",
            use_demo=("demo" in model_args.few_shot_type),
        )
        if training_args.do_predict
        else None
    ...
    def compute_metrics_fn(p: EvalPrediction):
        ...
        return acc_and_f1(preds, label_ids)
    ...
    if training_args.do_eval:
        ...
        for eval_dataset in eval_datasets:
            ...
            num_logits = output.predictions.shape[-1]
            logits = output.predictions.reshape(
                [eval_dataset.num_sample, -1, num_logits]
            ).mean(axis=0)

            np.save(
                os.path.join(training_args.output_dir, f"eval_truth.npy"),
                output.label_ids,
            )
            np.save(os.path.join(training_args.output_dir, f"eval_logits.npy"), logits)
            ...
            if trainer.is_world_process_zero():
                with open(output_eval_file, "w") as writer:
                    ...
                    for key, value in eval_result.items():
                        ...
                        writer.write("%s = %s\n" % (key, value))
                        final_result["dev_" + key] = value
            eval_results.update(eval_result)

    test_results = {}
    if training_args.do_predict:
        logging.info("*** Test ***")
        test_datasets = [test_dataset, test_competition_dataset]
        test_datasets_names = ["test", "test_official"]

        for d_ix, test_dataset in enumerate(test_datasets):
            trainer.compute_metrics = compute_metrics_fn
            ...
            if num_logits == 1:
                y_pred = np.squeeze(logits)
            else:
                y_pred = np.argmax(logits, axis=1)
            y_true = [example.label for example in test_dataset.query_examples]
            print(
            ...
            # Save results using competition format
            output_test_file_competition = os.path.join(
                training_args.output_dir,
                f"predictions_{test_datasets_names[d_ix]}.json",
            )
            with open(output_test_file_competition, "w") as writer:
                for e_ix, example in enumerate(test_dataset.query_examples):
                    writer.write(
                        f'{{"index": "{example.guid}", "prediction": {y_pred[e_ix]}, "truth": {example.label}}}\n'
                    )

            # Save logits + examples ids for later use (e.g. ensemble)
            example_ids = np.array([e.guid for e in test_dataset.query_examples])
            np.save(
                os.path.join(
                    training_args.output_dir,
                    f"{test_datasets_names[d_ix]}_example_ids.npy",
                ),
                example_ids,
            )
            np.save(
                os.path.join(
                    training_args.output_dir, f"{test_datasets_names[d_ix]}_logits.npy"
                ),
                logits,
            )

            output_test_file = os.path.join(
                training_args.output_dir, f"results_{test_datasets_names[d_ix]}.txt"
            )
            if trainer.is_world_process_zero():
