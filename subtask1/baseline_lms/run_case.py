...
"""\
Finetuning the library models for sequence classification on CASE 2022.
This is adapted from run_glue.py in Huggingface library. 
See: https://github.com/huggingface/transformers/blob/master/examples/pytorch/text-classification/run_glue.py
"""
...
import numpy as np
import transformers
from datasets import Features, Value, load_dataset, load_metric
...
ft = {"index": Value("string"), "text": Value("string"), "label": Value("int64")}
...
def main():
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        p, r, f, s = precision_recall_fscore_support(
            np.array(p.label_ids), np.array(preds), average="binary"
        )
        result["f1"] = f
        result["precision"] = p
        result["recall"] = r
